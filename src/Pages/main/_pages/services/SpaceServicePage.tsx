...
const SpaceServicePage = () => {
  ...
  const filteredServices = services.filter((service) => {
    const matchesSearch =
      ...
      service.features?.some((f: string) =>
        f.toLowerCase().includes(search.toLowerCase())
      );
    const matchesType = !filters.type || service.type === filters.type;
    const matchesCapacity =
      !filters.capacity || Number(service.capacity) >= Number(filters.capacity);
    const matchesMinPrice =
      !filters.minPrice ||
      Number(service.minCharge) >= Number(filters.minPrice);
  ...
  useEffect(() => {
    ...
    const fetchWorkspaceNames = async () => {
      const ids = Array.from(
        new Set(services.map((s) => s.workspaceId).filter(Boolean))
      );
        ...
        <section className="relative w-full max-w-7xl mx-auto py-8 mb-8 overflow-hidden rounded-2xl mt-10">
          ...
          <div className="relative z-30 flex flex-col items-center justify-center h-full text-center text-white px-4">
            ...
            <p className="text-lg md:text-xl mb-8 max-w-2xl leading-relaxed opacity-90">
              Discover inspiring workspaces designed for productivity,
              creativity, and collaboration in premium locations
            </p>
            <a
              href="#services"
        ...
        {showFilterModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 px-2">
            <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm relative animate-fade-in">
              ...
              <h2 className="text-2xl font-bold text-[#1D3A8A] mb-6 text-center">
                Filter Workspaces
              </h2>
              <form
                className="space-y-5"
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {filteredServices.map((service) => (
                <div
                  key={service.id}
                  className="group bg-white rounded-2xl shadow-SM hover:shadow-md transition-all duration-300 overflow-hidden border border-gray-100 transform hover:-translate-y-1 flex flex-col h-full"
                >
                  {/* Image Section */}
                  <div className="relative h-48 w-full overflow-hidden">
                    ...
                    {service.workspaceId &&
                      workspaceNames[service.workspaceId] && (
                        <div className="absolute bottom-3 left-3 bg-blue-100 flex items-center gap-2   text-[#214cc3] px-3 py-1 rounded-full text-xs font-semibold shadow-lg">
                          <Building2 className="w-5 h-5  felx" />{" "}
                          {workspaceNames[service.workspaceId]}
                        </div>
                      )}
                  </div>
                    ...
                    <div className="flex flex-wrap gap-2 mb-4">
                      {service.features
                        ...
                        .map((feature: string, index: number) => (
                          <div
                            key={index}
                            className="flex items-center bg-blue-50 text-blue-700 px-2 py-1 rounded-lg text-xs font-medium"
                          >
                      ...
                      <div className="flex justify-between items-center">
                        <div>
                          <p className="text-[#1D3A8A] font-bold text-lg">
                            ₦{service.minCharge?.toLocaleString()}/{" "}
