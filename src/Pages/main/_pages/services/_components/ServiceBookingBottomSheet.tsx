...
import { Calendar, Clock, Users, Tag, CheckCircle, AlertCircle, X, ChevronLeft, ChevronRight } from 'lucide-react';
...
const ServiceBookingBottomSheet: React.FC<ServiceBookingBottomSheetProps> = ({ open, onClose, service, onBook }) => {
  ...
  const { checkDiscountCode, applyDiscountUsage, createBooking, loading, error } = useBooking();
  const { user } = useUser();
  ...
  if (service.maxCharge && basePrice > service.maxCharge) basePrice = service.maxCharge;
  ...
  const handleCheckDiscount = async () => {
    ...
    const { discount, error: discountCheckError } = await checkDiscountCode(service.workspaceId, discountCode);
    setDiscount(discount);
    if (discount && discount.id === 'default') {
      setDiscountMessage('No discount code entered. A default 0% discount has been applied!');
      setDiscountAmount(0); // Default discount is 0% currently
    } else if (discountCheckError) {
      setDiscountError(discountCheckError);
      setDiscountAmount(0);
    } else if (discount) {
      setDiscountAmount(Math.round((basePrice * numSeats) * ((discount.percentage ?? 0) / 100)));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!canBook) {
      setFormError('Please fill all fields correctly and select a valid date/duration.');
      return;
    }
    if (!userId) {
    ...
    if (discountCode) {
      const { discount, error: discountCheckError } = await checkDiscountCode(service.workspaceId, discountCode);
      if (discount) {
        finalTotalPrice = basePrice * numSeats - Math.round((basePrice * numSeats) * ((discount.percentage ?? 0) / 100));
  ...
  return (
    ...
    >
      ...
      >
        <div className="bg-white rounded-t-[50px] shadow-2xl w-full max-h-[92vh] max-w-7xl mx-auto relative overflow-hidden">
          {/* Header */}
            ...
            <div className="flex items-center justify-center mb-8">
              <div className="flex items-center space-x-4">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-colors ${
                  step === 0 ? 'bg-[#1D3A8A] text-white' : 'bg-gray-200 text-gray-600'
                }`}>
                  1
                </div>
                <div className={`w-12 h-0.5 transition-colors ${step === 1 ? 'bg-[#1D3A8A]' : 'bg-gray-200'}`}></div>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-colors ${
                  step === 1 ? 'bg-[#1D3A8A] text-white' : 'bg-gray-200 text-gray-600'
                }`}>
                  2
                </div>
              </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <input
                        ...
                        required
                      />
                      <p className="text-xs text-gray-500">Business hours: {openTime} - {closeTime}</p>
                      {startTime && (startTime < openTime || startTime >= closeTime) && (
                        <div className="flex items-center gap-1 text-xs text-red-500">
                          <AlertCircle size={12} />
                    ...
                    <div className="space-y-2">
                      ...
                      <input
                        ...
                        required
                      />
                      <p className="text-xs text-gray-500">Range: {minDuration} - {maxDuration} {durationUnit}</p>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <input
                          ...
                          value={numSeats}
                          onChange={e => handleNumSeatsChange(Number(e.target.value))}
                          required
                        />
                        <button
                          type="button"
                      ...
                      <p className="text-xs text-gray-500">Maximum capacity: {service.capacity}</p>
                    </div>
                  </div>
                  ...
                  <div className="bg-gray-50 rounded-xl p-4 space-y-3">
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">{date} ({selectedDay})</span>
                      </div>
                      <div className="flex justify-between">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">₦{(basePrice * numSeats).toLocaleString()}</span>
                      </div>
                      {discountAmount > 0 && (
              ...
              <div className="flex justify-between pt-4 border-t">
                ...
                {step === 1 && (
                  <button 
                    ...
                    disabled={!canBook || loading || (!!discountCode && !!discountError)}
                  >
                    {loading ? (
                      <>
